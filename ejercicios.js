...
function encontrarMaximo(numeros) {
  ...
  if (numeros.length === 0) return undefined; // O un error, dependiendo de la especificación
...
function obtenerColumna(matriz, indiceColumna) {
  ...
  const columna = [];
  // Asumiendo que la matriz tiene al menos una fila y que todas las filas tienen el mismo número de columnas
  if (matriz.length > 0 && indiceColumna >= 0 && indiceColumna < matriz[0].length) {
    for (const fila of matriz) {
...
function transponer(matriz) {
  ...
  // Recorrer las 'nuevas filas' (que son las 'viejas columnas')
  for (let j = 0; j < columnas; j++) {
    const nuevaFila = [];
    // Recorrer las 'nuevas columnas' (que son las 'viejas filas')
    for (let i = 0; i < filas; i++) {
